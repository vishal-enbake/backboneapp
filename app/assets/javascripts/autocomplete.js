...
$.Autocompleter = function(input, options) {
  ...
  function selectCurrent() {
    ...
    var username = $(selected.result).filter("a").html();
    var matched_names_field = '';
    
    matched_names_field = $(input).parent().parent().next().find("input[name='form[matched_names]']");
    matched_names_field.val( matched_names_field.val() + username + ':' + options.ids[username]+ ',' );
  ...
  function onChange(crap, skipPrevCheck) {
    ...
    if ( currentValue.length >= options.minChars) {
      ...
      if (options.startmsg != null) {
        select.emptyList();
        select.display({}, null);
        if ($("."+options.inputClass).html() !=  ''){
        	var text = $("."+options.inputClass).html();
        	if(text.search('@') != -1 ){
        	select.show();
        	}
        }
...
$.Autocompleter.Select = function (options, input, select, config) {
  ...
  function init() {
    ...
    var comment_box = $(input);
    
    if( comment_box.hasClass('main_post_box') ){
	results_class = "main-post-box-results";
    }else if( comment_box.hasClass('post_comment_box') ){
	results_class = "ac_results_comments";
    }else if( comment_box.hasClass('pin_comment_box') ){
	results_class = "ac_results_pin";
    }else if( comment_box.hasClass('board_comment_box') ){
	results_class = "ac_results_boards_cmts";
    }else{}

    element = $("<div/>").hide().addClass(options.resultsClass + " " + results_class).insertAfter($(input).parent().parent().parent());
  ...
  function fillList(q) {
    ...
    if (options.startmsg && max == 100 && q == null) {
